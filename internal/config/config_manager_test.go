...
func TestConfigManager_LoadAndSave(t *testing.T) {
	...
	if config.Settings.DefaultBaseImage != "buildpack-deps:bookworm" {
		t.Errorf("Expected default base image 'buildpack-deps:bookworm', got %q", config.Settings.DefaultBaseImage)
	}
